...
export async function POST(request: NextRequest) {
  try {
    ...
    if (pago.externalReference) {
      ...
      if (externalRef.startsWith("MENSUAL-")) {
        const colegiadoId = externalRef.replace("MENSUAL-", "")
        console.log("[MP Webhook] Pago de mensualidad para colegiado:", colegiadoId)

        if (estado === "Aprobado") {
          const { error: errUpdate } = await supabase
            .from("pagos_mensualidades")
            .update({ estado: "Pagado", fecha_pago: new Date().toISOString() })
            .eq("colegiado_id", colegiadoId)
            .eq("estado", "Pendiente")

          if (errUpdate) {
            console.warn("[MP Webhook] Error al actualizar mensualidades:", errUpdate.message)
          } else {
            console.log("[MP Webhook] Mensualidades pagadas correctamente")
          }
        }
      } else {
